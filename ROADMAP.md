...
## 🚧 Phase 4.5 — 極致像素風視覺升級（方案 D）

為了徹底解決 SD 高清圖片在 16px 網格下縮放模糊、去背不完全及視覺不協調的問題，專案於 2026-06-28 決定調整視覺路線為 **方案 D（極致原生像素風）**，將像素美術做到極致：

- [ ] **精靈矩陣升級**：將現有 8x8 ~ 12x12 的像素精靈統一重構為 16x16 高規矩陣，增加色彩豐富度與精緻度
- [ ] **動態動畫幀**：為所有怪物（行走/受擊/死亡）與砲台（待機/攻擊）實作 2 ~ 4 幀的動態像素動畫
- [ ] **地面與場景紋理**：用精緻的像素地板 Tile（草地、石磚、水面、泥土）替換現有的純色背景網格，營造真實戰場感
- [ ] **像素風 UI 重構**：重製像素風格的 HUD 面板、血條、飄字與波次公告動畫
- [ ] **光影後處理與特效**：新增 Canvas 全局 Bloom（發光）濾鏡、暗角效果與屬性光暈，提升畫面現代感與高級感
- [ ] **清理 SD 冗餘**：在確認像素美術升級完成後，移除 SD 載入邏輯與 assets 中的高解析度去背白邊圖
