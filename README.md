...
本專案採用 **Vibecoding** 快速迭代開發模式，在多個 AI 模型的接力與協作下逐步完善。以下為本專案開發中曾使用過的 AI 模型與其對應的貢獻歷程：

1. **Claude 3.5 Sonnet**
   - **職責**：專案架構初創與核心複雜算法設計。
   - **核心貢獻**：實現了分幀 A* 動態尋路、雙向物理至邏輯座標映射、2.5D 像素渲染管道、以及核心五行相生相剋與陰陽雙路塔合成邏輯。

2. **Gemini 系列 (包括 Gemini 1.5 Pro / Gemini 3.5 Flash / Gemini 1.5 Flash)**
   - **職責**：大規模重構、核心功能擴展與細節打磨。
   - **核心貢獻**：
     - **天賦圖譜與經絡系統**：實作了 35+ 個天賦節點、羊皮紙古籍 tab UI，並手繪 inline SVG 精準映射人體穴位與流動金光路徑。
     - **戰鬥 UI 視覺重構**：新增了動態 SVG 波次進度環、怪物即時預覽與 Tower DPS 細節展示面板。
     - **系統優化**：重構 OffscreenCanvas 預渲染以降低 draw calls (-39.2%)；實作 HTML5 Audio Pool 靜態對象池避免瀏覽器媒體實例洩漏。
     - **P3 階段與擴展**：實作了 8 大「下次目標系統」(Goal System)、跨局挑戰記錄板、圖鑑與成就系統 (Bestiary)、以及 16 格草地自動拼接 (Autotiling) 技術。

3. **OpenCode Fusion (內建 GLM / Kimi 面板)**
   - **職責**：代碼安全覆審、去敏感化研究與多視角架構優化。
   - **核心貢獻**：透過安全邊界下的去敏感分析派發，引導本機 Vitest 測試驗證，確保在外部 API 缺失時也能順暢建置。

4. **Stable Diffusion (本機腳本批次生成)**
   - **職責**：生成原始像素精靈與地形美術材質。
   - **核心貢獻**：藉由輔助腳本批次生成 72×96 高解析度防禦塔圖示、草叢、石岸等無縫平鋪的明亮山水地形材質板（作為 runtime 的 fallback 與參考稿）。

5. **Gemini 3.5 Flash (Medium)**
   - **職責**：專案規模代碼分析、文檔整理、代碼清理與重構。
   - **核心貢獻**：
     - **專案規模統計**：對專案所有原始碼（包括 TypeScript、CSS、HTML、Python 等）進行了精確的行數與檔案數量統計。
     - **README 結構重構**：移除了 README.md 檔案中重複的舊版冗餘段落（例如重複的技術規格、本地開發、OpenCode 專案交接與專案結構），將其整合為單一、乾淨的最新版本。
     - **編碼問題修復**：解決了 README.md 在 UTF-8 解析時的無效字元問題，使專案文檔編碼更加規範與標準。
